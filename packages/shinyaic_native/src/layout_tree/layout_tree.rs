...
use crate::layout_tree::layout_object::LayoutObject;
...
impl LayoutObject {
    pub fn new(dom: DOMNode, cssom: CSSOM) -> Self {
        Self {
            dom,
            cssom,
            tree: LayoutObject::new(),
            window_size: WindowSize::new(900.0, 700.0),
        }
    }

    pub fn prepare_iterator(&self) -> Vec<LayoutObject> {
    ...
    pub fn layouting(&mut self, window_with: f32, window_height: f32) {
        let mut root_node = match self.tree {
            LayoutObject::ViewPort(ref mut viewport) => viewport,
            _ => panic!("TODO"),
        };
    ...
    fn traverse_single_dom(
        ...
        match dom_node.clone().node_type {
            NodeType::TextNode(txt) => LayoutObject::init_with_text(txt, None, font),
            NodeType::DomNode(element_type) => {
                match element_type.tag_name {
                    HTMLElements::StyleElement => {
                ...
                let raw_render_object = LayoutObject::init_with_element(
                    element_type,
                ...
                let current_font = match raw_render_object.clone() {
                    LayoutObject::Text(_) => font,
                    LayoutObject::ViewPort(render_object)
                    | LayoutObject::Scroll(render_object)
                    | LayoutObject::Block(render_object)
                    | LayoutObject::Inline(render_object) => {
...
mod test {
    use super::*;
    use crate::render_tree::render_object::_LayoutObject;
    ...
    fn test_render_tree() {
        ...
        let mut render_tree = LayoutObject::new(dom, cssom);
        ...
        assert_eq!(
            render_tree.tree,
            LayoutObject::ViewPort(_LayoutObject {
                rectangle: Rectangle {
                    x: 0.0,
                    y: 0.0,
                    width: 900.0,
                ...
                children: vec![LayoutObject::Scroll(_LayoutObject {
                    rectangle: Rectangle {
                        x: 0.0,
                        y: 0.0,
                        width: 900.0,
                    ...
                    children: vec![
                        LayoutObject::Block(_LayoutObject {
                            rectangle: Rectangle {
                                x: 0.0,
                                y: 0.0,
                                width: 120.0,
                            ...
                            children: vec![LayoutObject::init_with_text(
